...
icon_bg = as_rgb(color_as_int(opts.color17))
...
def _draw_icon(screen: Screen, index: int) -> int:
    ...
    # Количество вкладок
    tab_count = len(tm.tabs)

    # Формируем новый значок с числом вкладок
    icon_with_count = f" {ICON.strip()} {tab_count} "

    fg, bg = screen.cursor.fg, screen.cursor.bg
    screen.cursor.fg = icon_fg
    screen.cursor.bg = icon_bg
    screen.draw(icon_with_count)
    screen.cursor.fg, screen.cursor.bg = fg, bg
    screen.cursor.x = len(icon_with_count)
...
def _draw_left_status(
    ...
    if not tab.is_active:
        screen.cursor.fg = as_rgb(color_as_int(opts.color8))  # Серый цвет
